...
/*
 * Copyright (C) University of Florida Phoenix Unmanned Aerial Vehicle Team
 * All rights reserved.
 *
 * @file	Autopilot_GPS.cpp
 * @brief	GPS receiver functions
 * 
*/
...
#include <Autopilot_Math/Autopilot_Math.h>
#include <Autopilot_Notify/Autopilot_Notify.h>
#include <GCS_MAVLink/GCS.h>
#include <Autopilot_BoardConfig/Autopilot_BoardConfig.h>
#include <climits>

#include "Autopilot_GPS_NOVA.h"
#include "Autopilot_GPS_ERB.h"
#include "Autopilot_GPS_GSOF.h"
#include "Autopilot_GPS_MTK.h"
#include "Autopilot_GPS_MTK19.h"
#include "Autopilot_GPS_NMEA.h"
#include "Autopilot_GPS_SBF.h"
#include "Autopilot_GPS_SBP.h"
#include "Autopilot_GPS_SBP2.h"
#include "Autopilot_GPS_SIRF.h"
#include "Autopilot_GPS_UBLOX.h"
#include "Autopilot_GPS_MAV.h"
#include "GPS_Backend.h"

#if HAL_WITH_UAVCAN
#include <Autopilot_BoardConfig/Autopilot_BoardConfig_CAN.h>
#include <Autopilot_UAVCAN/Autopilot_UAVCAN.h>
#include "Autopilot_GPS_UAVCAN.h"
#endif
...
const uint32_t Autopilot_GPS::_baudrates[] = {9600U, 115200U, 4800U, 19200U, 38400U, 57600U, 230400U};

// initialisation blobs to send to the GPS to try to get it into the
// right mode
const char Autopilot_GPS::_initialisation_blob[] = UBLOX_SET_BINARY MTK_SET_BINARY SIRF_SET_BINARY;

Autopilot_GPS *Autopilot_GPS::_singleton;

// table of user settable parameters
const Autopilot_Param::GroupInfo Autopilot_GPS::var_info[] = {
    // @Param: TYPE
    ...
    // @RebootRequired: True
    // @User: Advanced
    Autopilot_GROUPINFO("TYPE",    0, Autopilot_GPS, _type[0], HAL_GPS_TYPE_DEFAULT),
    ...
    // @RebootRequired: True
    // @User: Advanced
    Autopilot_GROUPINFO("TYPE2",   1, Autopilot_GPS, _type[1], 0),
    ...
    // @User: Advanced
    Autopilot_GROUPINFO("NAVFILTER", 2, Autopilot_GPS, _navfilter, GPS_ENGINE_AIRBORNE_4G),
    ...
    // @User: Advanced
    Autopilot_GROUPINFO("AUTO_SWITCH", 3, Autopilot_GPS, _auto_switch, 1),
    ...
    // @User: Advanced
    // @RebootRequired: True
    Autopilot_GROUPINFO("MIN_DGPS", 4, Autopilot_GPS, _min_dgps, 100),
    ...
    // @User: Advanced
    Autopilot_GROUPINFO("SBAS_MODE", 5, Autopilot_GPS, _sbas_mode, 2),
    ...
    // @Units: deg
    // @User: Advanced
    Autopilot_GROUPINFO("MIN_ELEV", 6, Autopilot_GPS, _min_elevation, -100),
    ...
    // @User: Advanced
    Autopilot_GROUPINFO("INJECT_TO",   7, Autopilot_GPS, _inject_to, GPS_RTK_INJECT_TO_ALL),
    ...
    // @User: Advanced
    Autopilot_GROUPINFO("SBP_LOGMASK", 8, Autopilot_GPS, _sbp_logmask, 0xFF00),
    ...
    // @RebootRequired: True
    // @User: Advanced
    Autopilot_GROUPINFO("RAW_DATA", 9, Autopilot_GPS, _raw_data, 0),
    ...
    // @User: Advanced
    Autopilot_GROUPINFO("GNSS_MODE", 10, Autopilot_GPS, _gnss_mode[0], 0),
    ...
    // @User: Advanced
    Autopilot_GROUPINFO("SAVE_CFG", 11, Autopilot_GPS, _save_config, 2),
    ...
    // @Bitmask: 0:GPS,1:SBAS,2:Galileo,3:Beidou,4:IMES,5:QZSS,6:GLOSNASS
    // @User: Advanced
    Autopilot_GROUPINFO("GNSS_MODE2", 12, Autopilot_GPS, _gnss_mode[1], 0),
    ...
    // @User: Advanced
    Autopilot_GROUPINFO("AUTO_CONFIG", 13, Autopilot_GPS, _auto_config, 1),
    ...
    // @Range: 50 200
    // @User: Advanced
    Autopilot_GROUPINFO("RATE_MS", 14, Autopilot_GPS, _rate_ms[0], 200),
    ...
    // @Range: 50 200
    // @User: Advanced
    Autopilot_GROUPINFO("RATE_MS2", 15, Autopilot_GPS, _rate_ms[1], 200),
    ...
    // @Range: -10 10
    // @User: Advanced
    Autopilot_GROUPINFO("POS1", 16, Autopilot_GPS, _antenna_offset[0], 0.0f),
    ...
    // @Range: -10 10
    // @User: Advanced
    Autopilot_GROUPINFO("POS2", 17, Autopilot_GPS, _antenna_offset[1], 0.0f),
    ...
    // @User: Advanced
    // @RebootRequired: True
    Autopilot_GROUPINFO("DELAY_MS", 18, Autopilot_GPS, _delay_ms[0], 0),
    ...
    // @User: Advanced
    // @RebootRequired: True
    Autopilot_GROUPINFO("DELAY_MS2", 19, Autopilot_GPS, _delay_ms[1], 0),
    ...
    // @User: Advanced
    Autopilot_GROUPINFO("BLEND_MASK", 20, Autopilot_GPS, _blend_mask, 5),
    ...
    // @Range: 5.0 30.0
    // @User: Advanced
    Autopilot_GROUPINFO("BLEND_TC", 21, Autopilot_GPS, _blend_tc, 10.0f),

    Autopilot_GROUPEND
};

// constructor
Autopilot_GPS::Autopilot_GPS()
{
    ...
    Autopilot_Param::setup_object_defaults(this, var_info);

    if (_singleton != nullptr) {
        Autopilot_HAL::panic("Autopilot_GPS must be singleton");
    }
    _singleton = this;
}

/// Startup initialisation.
void Autopilot_GPS::init(const Autopilot_SerialManager& serial_manager)
{
    primary_instance = 0;

    // search for serial ports with gps protocol
    _port[0] = serial_manager.find_serial(Autopilot_SerialManager::SerialProtocol_GPS, 0);
    _port[1] = serial_manager.find_serial(Autopilot_SerialManager::SerialProtocol_GPS, 1);
    _last_instance_swAutopilot_ms = 0;
...
uint8_t Autopilot_GPS::num_sensors(void) const
{
...
bool Autopilot_GPS::speed_accuracy(uint8_t instance, float &sacc) const
{
...
bool Autopilot_GPS::horizontal_accuracy(uint8_t instance, float &hacc) const
{
...
bool Autopilot_GPS::vertical_accuracy(uint8_t instance, float &vacc) const
{
 ...
 */
uint64_t Autopilot_GPS::time_epoch_convert(uint16_t gps_week, uint32_t gps_ms)
{
    uint64_t fix_time_ms = UNIX_OFFSET_MSEC + gps_week * Autopilot_MSEC_PER_WEEK + gps_ms;
    return fix_time_ms;
}
 ...
 */
uint64_t Autopilot_GPS::time_epoch_usec(uint8_t instance) const
{
    ...
    return (fix_time_ms + (Autopilot_HAL::millis() - istate.last_gps_time_ms)) * 1000ULL;
}
 ...
 */
void Autopilot_GPS::send_blob_start(uint8_t instance, const char *_blob, uint16_t size)
{
...
/*
  ...
  UART transmit buffer
 */
void Autopilot_GPS::send_blob_update(uint8_t instance)
{
 ...
 */
void Autopilot_GPS::detect_instance(uint8_t instance)
{
    Autopilot_GPS_Backend *new_gps = nullptr;
    struct detect_state *dstate = &detect_state[instance];
    const uint32_t now = Autopilot_HAL::millis();
    ...
    case GPS_TYPE_MAV:
        ...
        new_gps = new Autopilot_GPS_MAV(*this, state[instance], nullptr);
        goto found_gps;
        break;
...
#if HAL_WITH_UAVCAN
        dstate->auto_detected_baud = false; // specified, not detected
        new_gps = Autopilot_GPS_UAVCAN::probe(*this, state[instance]);
        goto found_gps;
#endif
    ...
    case GPS_TYPE_SBF:
        new_gps = new Autopilot_GPS_SBF(*this, state[instance], _port[instance]);
        break;

    case GPS_TYPE_GSOF:
        new_gps = new Autopilot_GPS_GSOF(*this, state[instance], _port[instance]);
        break;

    case GPS_TYPE_NOVA:
        new_gps = new Autopilot_GPS_NOVA(*this, state[instance], _port[instance]);
        break;
    ...
    if (now - dstate->last_baud_change_ms > GPS_BAUD_TIME_MS) {
        ...
        _port[instance]->set_flow_control(Autopilot_HAL::UARTDriver::FLOW_CONTROL_DISABLE);
    ...
    while (initblob_state[instance].remaining == 0 && _port[instance]->available() > 0
        ...
        */
        if ((_type[instance] == GPS_TYPE_AUTO || _type[instance] == GPS_TYPE_UBLOX) &&
            ...
            Autopilot_GPS_UBLOX::_detect(dstate->ublox_detect_state, data)) {
            new_gps = new Autopilot_GPS_UBLOX(*this, state[instance], _port[instance]);
        }
        ...
        // and are surprisingly large
                 ...
                 Autopilot_GPS_MTK19::_detect(dstate->mtk19_detect_state, data)) {
            new_gps = new Autopilot_GPS_MTK19(*this, state[instance], _port[instance]);
        } else if ((_type[instance] == GPS_TYPE_AUTO || _type[instance] == GPS_TYPE_MTK) &&
                   Autopilot_GPS_MTK::_detect(dstate->mtk_detect_state, data)) {
            new_gps = new Autopilot_GPS_MTK(*this, state[instance], _port[instance]);
        }
#endif
        else if ((_type[instance] == GPS_TYPE_AUTO || _type[instance] == GPS_TYPE_SBP) &&
                 Autopilot_GPS_SBP2::_detect(dstate->sbp2_detect_state, data)) {
            new_gps = new Autopilot_GPS_SBP2(*this, state[instance], _port[instance]);
        }
        else if ((_type[instance] == GPS_TYPE_AUTO || _type[instance] == GPS_TYPE_SBP) &&
                 Autopilot_GPS_SBP::_detect(dstate->sbp_detect_state, data)) {
            new_gps = new Autopilot_GPS_SBP(*this, state[instance], _port[instance]);
        }
#if !HAL_MINIMIZE_FEATURES
        else if ((_type[instance] == GPS_TYPE_AUTO || _type[instance] == GPS_TYPE_SIRF) &&
                 Autopilot_GPS_SIRF::_detect(dstate->sirf_detect_state, data)) {
            new_gps = new Autopilot_GPS_SIRF(*this, state[instance], _port[instance]);
        }
#endif
        else if ((_type[instance] == GPS_TYPE_AUTO || _type[instance] == GPS_TYPE_ERB) &&
                 Autopilot_GPS_ERB::_detect(dstate->erb_detect_state, data)) {
            new_gps = new Autopilot_GPS_ERB(*this, state[instance], _port[instance]);
        } else if (_type[instance] == GPS_TYPE_NMEA &&
                   Autopilot_GPS_NMEA::_detect(dstate->nmea_detect_state, data)) {
            new_gps = new Autopilot_GPS_NMEA(*this, state[instance], _port[instance]);
        }
    }
...
Autopilot_GPS::GPS_Status Autopilot_GPS::highest_supported_status(uint8_t instance) const
{
    if (instance < GPS_MAX_RECEIVERS && drivers[instance] != nullptr) {
        return drivers[instance]->highest_supported_status();
    }
    return Autopilot_GPS::GPS_OK_FIX_3D;
}

bool Autopilot_GPS::should_df_log() const
{
    Autopilot_Logger *instance = Autopilot_Logger::get_singleton();
    if (instance == nullptr) {
        return false;
    }
 ...
 */
void Autopilot_GPS::update_instance(uint8_t instance)
{
    ...
    uint32_t tnow = Autopilot_HAL::millis();
    ...
    if (state[instance].status >= GPS_OK_FIX_3D) {
        ...
        AP::rtc().set_utc_usec(now, Autopilot_RTC::SOURCE_GPS);
    }
}
 ...
 */
void Autopilot_GPS::update(void)
{
    for (uint8_t i=0; i<GPS_MAX_RECEIVERS; i++) {
        update_instance(i);
    }
            ...
            if (primary_instance == GPS_BLENDED_INSTANCE) {
                ...
                for (uint8_t i=1; i<GPS_MAX_RECEIVERS; i++) {
                    ...
                    if ((state[i].status > state[primary_instance].status) ||
                        ...
                        primary_instance = i;
                        _last_instance_swAutopilot_ms = now;
                    }
                }
            } else {
                ...
                for (uint8_t i=0; i<GPS_MAX_RECEIVERS; i++) {
                    ...
                    if (state[i].status == state[primary_instance].status && another_gps_has_1_or_more_sats) {
                        ...
                        if ((another_gps_has_1_or_more_sats && (now - _last_instance_swAutopilot_ms) >= 20000) ||
                            (another_gps_has_2_or_more_sats && (now - _last_instance_swAutopilot_ms) >= 5000)) {
    ...
    Autopilot_Notify::flags.gps_status = state[primary_instance].status;
    Autopilot_Notify::flags.gps_num_sats = state[primary_instance].num_sats;

}

void Autopilot_GPS::handle_gps_inject(const mavlink_message_t *msg)
{
 ...
 */
void Autopilot_GPS::handle_msg(const mavlink_message_t *msg)
{
    switch (msg->msgid) {
 ...
 */
void Autopilot_GPS::setHIL(uint8_t instance, GPS_Status _status, uint64_t time_epoch_ms,
                    ...
                    uint16_t hdop)
{
    ...
    const uint32_t tnow = Autopilot_HAL::millis();
    ...
    istate.ground_course = wrAutopilot_360(degrees(atan2f(istate.velocity.y, istate.velocity.x)));
    istate.hdop = hdop;
    istate.num_sats = _num_sats;
    istate.last_gps_time_ms = tnow;
    uint64_t gps_time_ms = time_epoch_ms - UNIX_OFFSET_MSEC;
    istate.time_week     = gps_time_ms / Autopilot_MSEC_PER_WEEK;
    istate.time_week_ms  = gps_time_ms - istate.time_week * Autopilot_MSEC_PER_WEEK;
    timing[instance].last_message_time_ms = tnow;
    timing[instance].last_fix_time_ms = tnow;
...
void Autopilot_GPS::setHIL_Accuracy(uint8_t instance, float vdop, float hacc, float vacc, float sacc, bool _have_vertical_velocity, uint32_t sample_ms)
{
    if (instance >= GPS_MAX_RECEIVERS) {
        return;
    }
 ...
 */
void Autopilot_GPS::lock_port(uint8_t instance, bool lock)
{
...
void Autopilot_GPS::inject_data(uint8_t *data, uint16_t len)
{
...
void Autopilot_GPS::inject_data(uint8_t instance, uint8_t *data, uint16_t len)
{
    if (instance < GPS_MAX_RECEIVERS && drivers[instance] != nullptr) {
...
void Autopilot_GPS::send_mavlink_gps_raw(mavlink_channel_t chan)
{
    static uint32_t last_send_time_ms[MAVLINK_COMM_NUM_BUFFERS];
    if (status(0) > Autopilot_GPS::NO_GPS) {
...
void Autopilot_GPS::send_mavlink_gps2_raw(mavlink_channel_t chan)
{
    static uint32_t last_send_time_ms[MAVLINK_COMM_NUM_BUFFERS];
    if (num_instances < 2 || status(1) <= Autopilot_GPS::NO_GPS) {
        return;
    }
    // when we have a GPS then only send new data
...
void Autopilot_GPS::send_mavlink_gps_rtk(mavlink_channel_t chan, uint8_t inst)
{
...
uint8_t Autopilot_GPS::first_unconfigured_gps(void) const
{
...
void Autopilot_GPS::broadcast_first_configuration_failure_reason(void) const
{
...
bool Autopilot_GPS::all_consistent(float &distance) const
{
...
bool Autopilot_GPS::blend_health_check() const
{
 ...
 */
void Autopilot_GPS::handle_gps_rtcm_data(const mavlink_message_t *msg)
{
...
void Autopilot_GPS::Write_Autopilot_Logger_Log_Startup_messages()
{
    for (uint8_t instance=0; instance<num_instances; instance++) {
        if (drivers[instance] == nullptr || state[instance].status == NO_GPS) {
            continue;
        }
        drivers[instance]->Write_Autopilot_Logger_Log_Startup_messages();
    }
}
 ...
 */
bool Autopilot_GPS::get_lag(uint8_t instance, float &lag_sec) const
{
...
const Vector3f &Autopilot_GPS::get_antenna_offset(uint8_t instance) const
{
    if (instance == GPS_MAX_RECEIVERS) {
...
*/
uint16_t Autopilot_GPS::get_rate_ms(uint8_t instance) const
{
    // sanity check
...
*/
bool Autopilot_GPS::calc_blend_weights(void)
{
...
*/
void Autopilot_GPS::calc_blended_state(void)
{
    ...
    */
    ...
    state[GPS_BLENDED_INSTANCE].ground_course = wrAutopilot_360(degrees(atan2f(state[GPS_BLENDED_INSTANCE].velocity.y, state[GPS_BLENDED_INSTANCE].velocity.x)));
...
bool Autopilot_GPS::is_healthy(uint8_t instance) const {
    return drivers[instance] != nullptr &&
           last_message_delta_time_ms(instance) < GPS_MAX_DELTA_MS &&
           drivers[instance]->is_healthy();
}

bool Autopilot_GPS::prepare_for_arming(void) {
...
Autopilot_GPS &gps()
{
    return Autopilot_GPS::get_singleton();
}
