 ...
 * All rights reserved.
 *
 * @file	Autopilot_GPS_NOVA.cpp
 * @brief	GPS drivers derived from Novatel/ Tersus/ ComNav
 * 
*/
...
#include <Autopilot_Logger/Autopilot_Logger.h>
...
Autopilot_GPS_NOVA::Autopilot_GPS_NOVA(Autopilot_GPS &_gps, Autopilot_GPS::GPS_State &_state,
                       Autopilot_HAL::UARTDriver *_port) :
    Autopilot_GPS_Backend(_gps, _state, _port)
{
...
bool
Autopilot_GPS_NOVA::parse(uint8_t temp)
{
...
bool
Autopilot_GPS_NOVA::process_message(void)
{
    ...
    if (messageid == 42) // bestpos
    {
        ...
        state.last_gps_time_ms = Autopilot_HAL::millis();
...
void
Autopilot_GPS_NOVA::inject_data(const uint8_t *data, uint16_t len)
{
    if (port->txspace() > len) {
        last_injected_data_ms = Autopilot_HAL::millis();
        port->write(data, len);
    } else {
...
uint32_t Autopilot_GPS_NOVA::CRC32Value(uint32_t icrc)
{
    int i;
    uint32_t crc = icrc;
    for ( i = 8 ; i > 0; i-- )
...
uint32_t Autopilot_GPS_NOVA::CalculateBlockCRC32(uint32_t length, uint8_t *buffer, uint32_t crc)
{
    while ( length-- != 0 )
    {
